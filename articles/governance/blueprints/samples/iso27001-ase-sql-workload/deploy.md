...
|Azure SQL Database template|Resource Manager template|Azure SQL Server admin password (Key Vault Resource ID)|The Resource ID of the Key Vault. Use "/subscription/{subscriptionId}/resourceGroups/{orgName}-workload-kv-rg/providers/Microsoft.KeyVault/vaults/{orgName}-workload-kv" and replace `{subscriptionId}` with your Subscription ID and `{orgName}` with the **Organization name** blueprint parameter.|
